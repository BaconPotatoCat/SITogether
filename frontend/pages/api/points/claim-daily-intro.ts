...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const backendUrl = `${config.backendInternalUrl}/api/points/claim-daily-intro`
    ...
    console.error('Failed to claim daily intro points:', error)
    res.status(500).json({
      success: false,
      error: `Failed to claim daily intro points: ${error instanceof Error ? error.message : 'Unknown error'}`,
